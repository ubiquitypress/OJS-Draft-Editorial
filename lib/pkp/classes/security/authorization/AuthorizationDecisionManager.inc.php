...
class AuthorizationDecisionManager {
	/** @var PolicySet the xxxxxx policy set */
	var $_xxxxxxPolicySet;
	...
	function AuthorizationDecisionManager() {
		// Instantiate the main policy set we'll add xxxxxx policies to.
		$this->_xxxxxxPolicySet = new PolicySet(COMBINING_DENY_OVERRIDES);
	}
	 ...
	 * policies in the xxxxxx policy set applies.
	 * @param $decisionIfNoPolicyApplies integer
	 */
	function setDecisionIfNoPolicyApplies($decisionIfNoPolicyApplies) {
		$this->_xxxxxxPolicySet->setEffectIfNoPolicyApplies($decisionIfNoPolicyApplies);
	}
	...
	function addPolicy(&$policyOrPolicySet, $addToTop = false) {
		$this->_xxxxxxPolicySet->addPolicy($policyOrPolicySet, $addToTop);
	}
	 ...
	 */
	function decide() {
		// Decide the xxxxxx policy set which will recursively decide
		// all nested policy sets and return a single decision.
		$callOnDeny = null;
		$decision = $this->_decidePolicySet($this->_xxxxxxPolicySet, $callOnDeny);
