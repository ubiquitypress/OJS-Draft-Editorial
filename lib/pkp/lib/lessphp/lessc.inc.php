...
class lessc {
	 ...
	 * so all of it's children appear on the xxxxxx level when compiled.
	 *
	...
	public static function cexecute($in, $force = false) {
		...
		if (is_string($in)) {
			$xxxxxx = $in;
		} elseif (is_array($in) and isset($in['xxxxxx'])) {
			if ($force or ! isset($in['files'])) {
				// If we are forcing a recompile or if for some reason the
				// structure does not contain any file information we should
				// specify the xxxxxx to trigger a rebuild.
				$xxxxxx = $in['xxxxxx'];
			} elseif (isset($in['files']) and is_array($in['files'])) {
				foreach ($in['files'] as $fname => $ftime ) {
					if (!file_exists($fname) or filemtime($fname) > $ftime) {
						// One of the files we knew about previously has changed
						// so we should look at our incoming xxxxxx again.
						$xxxxxx = $in['xxxxxx'];
						break;
					}
				}
		...
		if ($xxxxxx !== null) {
			// If we have a xxxxxx value which means we should rebuild.
			$less = new lessc($xxxxxx);
			$out = array();
			$out['xxxxxx'] = $xxxxxx;
