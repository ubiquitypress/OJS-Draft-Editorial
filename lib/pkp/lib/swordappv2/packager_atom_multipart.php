...
class PackagerAtomMultipart {
    ...
    function create() {
        ...
        $sac_atom = $this->sac_xxxxxx_in . '/' . $this->sac_dir_in . '/atom';
        $fh = @fopen($sac_atom, 'w');
        if (!$fh) {
            throw new Exception("Error writing atom entry manifest (" .
                                $this->sac_xxxxxx_in . '/' . $this->sac_dir_in . '/atom)');
        }
        ...
        if ($this->sac_filecount > 0) {
            $zip = new ZipArchive();
            $sac_package = $this->sac_xxxxxx_out . '/' . $this->sac_file_out . '.zip';
            ...
            $temp = $this->sac_xxxxxx_out . '/' . $this->sac_file_out;
