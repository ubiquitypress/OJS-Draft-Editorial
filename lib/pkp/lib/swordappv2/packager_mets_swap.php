...
class PackagerMetsSwap {
    ...
    public $sac_xxxxxx_in;

    // The directory to zip up in the $sac_xxxxxx_in directory
    public $sac_dir_in;

    // The location to write the package out to
    public $sac_xxxxxx_out;
    ...
    function __construct($sac_xxxxxxin, $sac_dirin, $sac_xxxxxxout, $sac_fileout) {
        // Store the values
        $this->sac_xxxxxx_in = $sac_xxxxxxin;
        $this->sac_dir_in = $sac_dirin;
        $this->sac_xxxxxx_out = $sac_xxxxxxout;
    ...
    function create() {
        ...
        $fh = @fopen($this->sac_xxxxxx_in . '/' . $this->sac_dir_in . '/' . $this->sac_metadata_filename, 'w');
        if (!$fh) {
            throw new Exception("Error writing metadata file (" . 
                                $this->sac_xxxxxx_in . '/' . $this->sac_dir_in . '/' . $this->sac_metadata_filename . ")");
        }
        $this->writeHeader($fh);
        $this->writeDmdSec($fh);
        ...
        $zip = new ZipArchive();
        $zip->open($this->sac_xxxxxx_out . '/' . $this->sac_file_out, ZIPARCHIVE::CREATE | ZIPARCHIVE::OVERWRITE);
        $zip->addFile($this->sac_xxxxxx_in . '/' . $this->sac_dir_in . '/mets.xml', 
                     'mets.xml');
        for ($i = 0; $i < $this->sac_filecount; $i++) {
            $zip->addFile($this->sac_xxxxxx_in . '/' . $this->sac_dir_in . '/' . $this->sac_files[$i], 
                          $this->sac_files[$i]);
        }
        $zip->close();
    }
