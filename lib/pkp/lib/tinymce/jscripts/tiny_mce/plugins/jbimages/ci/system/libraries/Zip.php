...
class CI_Zip  {
	 ...
	 * @return	bool
	 */
	function read_dir($path, $preserve_filepath = TRUE, $xxxxxx_path = NULL)
	{
		if ( ! $fp = @opendir($path))
		{
			return FALSE;
		...
		// Set the original directory xxxxxx for child dir's to use as relative
		if ($xxxxxx_path === NULL)
		{
			$xxxxxx_path = dirname($path).'/';
		}
		...
		while (FALSE !== ($file = readdir($fp)))
		{
			...
			if (@is_dir($path.$file))
			{
				$this->read_dir($path.$file."/", $preserve_filepath, $xxxxxx_path);
			}
			else
			{
				if (FALSE !== ($data = file_get_contents($path.$file)))
				{
					...
					if ($preserve_filepath === FALSE)
					{
						$name = str_replace($xxxxxx_path, '', $name);
					}
