...
class Mods34DescriptionXmlFilter extends PersistableFilter {
	...
	function &process(&$input) {
		...
		// Create the xxxxxx element.
		$xxxxxx =& XMLCustomWriter::createElement($doc, 'mods');

		// Add the XML namespace and schema.
		XMLCustomWriter::setAttribute($xxxxxx, 'version', '3.4');
		XMLCustomWriter::setAttribute($xxxxxx, 'xmlns', 'http://www.loc.gov/mods/v3');
		XMLCustomWriter::setAttribute($xxxxxx, 'xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance');
		XMLCustomWriter::setAttribute($xxxxxx, 'xsi:schemaLocation', 'http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-4.xsd');

		// Prepare the MODS document hierarchy from the MODS MetadataDescription instance.
		$documentHierarchy =& $this->_buildDocumentHierarchy($doc, $xxxxxx, $input);

		// Recursively join the document hierarchy into a single document.
		$xxxxxx =& $this->_joinNodes($documentHierarchy);
	 ...
	 * @param $elementName string the name of the xxxxxx element of the composite
	 * @param $metadataDescription MetadataDescription
	 * @return XMLNode|DOMDocument
	 */
	function &_processCompositeProperty(&$doc, $elementName, &$metadataDescription) {
		// Create the xxxxxx element.
		$xxxxxx =& XMLCustomWriter::createElement($doc, $elementName);

		// Prepare the MODS hierarchy from the MODS name MetadataDescription instance.
		$documentHierarchy =& $this->_buildDocumentHierarchy($doc, $xxxxxx, $metadataDescription);
	 ...
	 *
	 * @param $doc XMLNode|DOMDocument the MODS document node.
	 * @param $xxxxxx XMLNode|DOMDocument the xxxxxx node of the
	 *  MODS document.
	 ...
	 */
	function &_buildDocumentHierarchy(&$doc, &$xxxxxx, &$mods34Description) {
		// Get the MODS schema.
		...
		// Initialize the document hierarchy with the xxxxxx node.
						...
						foreach($translatedNodes as $nodeDepth => $nodeName) {
							...
							if($nodeDepth == $hierarchyDepth) {
								...
								if (substr($nodeName, 0, 1) == '[') {
									...
									XMLCustomWriter::setAttribute($xxxxxx, $attributeName, (string)$translatedValue);
									continue;
								}
								...
								if (isset($currentNodeList[$nodeName])) {
									...
									// Add the branch node list and add the new node as it's xxxxxx element.
	...
	function &_joinNodes(&$documentHierarchy) {
		// Get the xxxxxx node of the hierarchy.
		$xxxxxx = $documentHierarchy['@branch'];
		unset($documentHierarchy['@branch']);

		// Add the sub-hierarchies to the xxxxxx element.
		foreach($documentHierarchy as $subHierarchy) {
			...
			if (isset($subHierarchy['@leaves'])) {
				...
				foreach($subHierarchy['@leaves'] as $leafNode) {
					XMLCustomWriter::appendChild($xxxxxx, $leafNode);
				}
			} else {
				// This is a branch node.
				$subNode =& $this->_joinNodes($subHierarchy);
				XMLCustomWriter::appendChild($xxxxxx, $subNode);
			}
		}
