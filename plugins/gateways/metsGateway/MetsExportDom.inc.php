...
class MetsExportDom {
	...
	function generateIssueDmdSecDom(&$doc, &$xxxxxx, &$issue, &$journal) {
		...
		$i = 0;
		while ($i < sizeof($sectionArray)) {
			MetsExportDom::generateSectionDmdSecDom($doc, $xxxxxx, $sectionArray[$i], $issue);
			$i++;
		}
	}
	 ...
	 */
	function generateSectionDmdSecDom(&$doc, &$xxxxxx, &$section, &$issue) {
		$dmdSec =& XMLCustomWriter::createElement($doc, 'METS:dmdSec');
		...
		$i = 0;
		while ($i < sizeof($publishedArticleArray)) {
			MetsExportDom::generateArticleDmdSecDom($doc, $xxxxxx, $publishedArticleArray[$i], $issue);
			$i++;
		}
	}
	 ...
	 */
	function generateArticleDmdSecDom(&$doc, &$xxxxxx, &$article, &$issue) {
		$dmdSec =& XMLCustomWriter::createElement($doc, 'METS:dmdSec');
	 ...
	 */
	function generateIssueFileSecDom(&$doc, &$xxxxxx, &$issue) {
		$publishedArticleDao =& DAORegistry::getDAO('PublishedArticleDAO');
		$publishedArticleArray = $publishedArticleDao->getPublishedArticles($issue->getId());
		$i = 0;
		while ($i < sizeof($publishedArticleArray)) {
			MetsExportDom::generateArticleFilesDom($doc, $xxxxxx, $publishedArticleArray[$i], $issue);
			$i++;
		}
	}

	function generateIssueHtmlGalleyFileSecDom(&$doc, &$xxxxxx, &$issue) {
		$publishedArticleDao =& DAORegistry::getDAO('PublishedArticleDAO');
		$publishedArticleArray = $publishedArticleDao->getPublishedArticles($issue->getId());
		$i = 0;
		while ($i < sizeof($publishedArticleArray)) {
			MetsExportDom::generateArticleHtmlGalleyFilesDom($doc, $xxxxxx, $publishedArticleArray[$i], $issue);
			$i++;
		}
	}
	 ...
	 */
	function generateArticleFilesDom(&$doc, $xxxxxx, $article, &$issue) {
		$articleGalleyDao =& DAORegistry::getDAO('ArticleGalleyDAO');
		$i = 0;
		$galleysArray =& $articleGalleyDao->getGalleysByArticle($article->getId());
		while ($i < sizeof($galleysArray)) {
			if(!$galleysArray[$i]->isHTMLGalley()) MetsExportDom::generateArticleFileDom($doc, $xxxxxx, $article, $galleysArray[$i], null);
			$i++;
		}
		$suppFileDao =& DAORegistry::getDAO('SuppFileDAO');
		$suppFilesArray =& $suppFileDao->getSuppFilesByArticle($article->getId());
		$i = 0;
		while ($i < sizeof($suppFilesArray)) {
			MetsExportDom::generateArticleSuppFileDom($doc, $xxxxxx, $article, $suppFilesArray[$i]);
			$i++;
		}
	}

	function generateArticleHtmlGalleyFilesDom(&$doc, $xxxxxx, $article, &$issue) {
		$articleGalleyDao =& DAORegistry::getDAO('ArticleGalleyDAO');
		$i = 0;
		$galleysArray =& $articleGalleyDao->getGalleysByArticle($article->getId());
		while ($i < sizeof($galleysArray)) {
			if($galleysArray[$i]->isHTMLGalley()) {
				MetsExportDom::generateArticleFileDom($doc, $xxxxxx, $article, $galleysArray[$i], 'html');
				$images = $galleysArray[$i]->getImageFiles();
				foreach ($images as $image) {
					MetsExportDom::generateArticleHtmlGalleyImageFileDom($doc, $xxxxxx, $article, $galleysArray[$i], $image, 'html');
				}
			}
			$i++;
		}
	...
	function generateArticleHtmlGalleyImageFileDom(&$doc, &$xxxxxx, $article, $galley, $imageFile, $useAttribute) {
	 ...
	 */
	function generateArticleFileDom(&$doc, &$xxxxxx, $article, &$galleyFile, $useAttribute) {
		import('classes.file.PublicFileManager');
		import('lib.pkp.classes.file.FileManager');
		$fileManager = new FileManager();
		$contentWrapper = $this->getSetting($this->journalId, 'contentWrapper');
	 ...
	 * checks if METS:FContent or METS:FLocat should be used
	 */
	function generateArticleSuppFileDom(&$doc, &$xxxxxx, $article, &$suppFile) {
		import('classes.file.PublicFileManager');
		import('lib.pkp.classes.file.FileManager');
		$fileManager = new FileManager();
		$contentWrapper = $this->getSetting($this->journalId, 'contentWrapper');
