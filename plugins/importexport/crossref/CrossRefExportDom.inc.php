...
class CrossRefExportDom {
	...
	function &generateDoiBatchDom(&$doc) {
		...
		// Generate the xxxxxx node for the file first and set its attributes
		$xxxxxx =& XMLCustomWriter::createElement($doc, 'doi_batch');

		/* Root doi_batch tag attributes
		 * Change to these attributes must be accompanied by a review of entire output
		 */
		XMLCustomWriter::setAttribute($xxxxxx, 'xmlns:xsi', CROSSREF_XMLNS_XSI);
		XMLCustomWriter::setAttribute($xxxxxx, 'xmlns', CROSSREF_XMLNS);
		XMLCustomWriter::setAttribute($xxxxxx, 'version', CROSSREF_VERSION);
		XMLCustomWriter::setAttribute($xxxxxx, 'xsi:schemaLocation', CROSSREF_XSI_SCHEMALOCATION);
