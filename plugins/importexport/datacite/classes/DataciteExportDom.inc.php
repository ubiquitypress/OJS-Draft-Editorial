...
class DataciteExportDom extends DOIExportDom {
	...
	function &generate(&$object) {
		...
		// Create the XML document and its xxxxxx element.
		$doc =& $this->getDoc();
		$xxxxxxElement =& $this->xxxxxxElement();
		XMLCustomWriter::appendChild($doc, $xxxxxxElement);

		// DOI (mandatory)
		if (($identifierElement =& $this->_identifierElement($object)) === false) return false;
		XMLCustomWriter::appendChild($xxxxxxElement, $identifierElement);

		// Creators (mandatory)
		XMLCustomWriter::appendChild($xxxxxxElement, $this->_creatorsElement($object, $objectLocalePrecedence, $publisher));

		// Title (mandatory)
		XMLCustomWriter::appendChild($xxxxxxElement, $this->_titlesElement($object, $objectLocalePrecedence));

		// Publisher (mandatory)
		XMLCustomWriter::createChildWithText($this->getDoc(), $xxxxxxElement, 'publisher', $publisher);

		// Publication Year (mandatory)
		XMLCustomWriter::createChildWithText($this->getDoc(), $xxxxxxElement, 'publicationYear', date('Y', strtotime($publicationDate)));

		// Subjects
		if (!empty($suppFile)) {
			$this->_appendNonMandatoryChild($xxxxxxElement, $this->_subjectsElement($suppFile, $objectLocalePrecedence));
		} elseif (!empty($article)) {
			$this->_appendNonMandatoryChild($xxxxxxElement, $this->_subjectsElement($article, $objectLocalePrecedence));
		}

		// Dates
		XMLCustomWriter::appendChild($xxxxxxElement, $this->_datesElement($issue, $article, $articleFile, $suppFile, $publicationDate));

		// Language
		XMLCustomWriter::createChildWithText($this->getDoc(), $xxxxxxElement, 'language', AppLocale::getIso1FromLocale($objectLocalePrecedence[0]));

		// Resource Type
		if (!is_a($object, 'SuppFile')) {
			$resourceTypeElement =& $this->_resourceTypeElement($object);
			XMLCustomWriter::appendChild($xxxxxxElement, $resourceTypeElement);
		}

		// Alternate Identifiers
		$this->_appendNonMandatoryChild($xxxxxxElement, $this->_alternateIdentifiersElement($object, $issue, $article, $articleFile));

		// Related Identifiers
		$this->_appendNonMandatoryChild($xxxxxxElement, $this->_relatedIdentifiersElement($object, $articlesByIssue, $galleysByArticle, $suppFilesByArticle, $issue, $article));

		// Sizes
		$sizesElement =& $this->_sizesElement($object, $article);
		if ($sizesElement) XMLCustomWriter::appendChild($xxxxxxElement, $sizesElement);

		// Formats
		if (!empty($articleFile)) XMLCustomWriter::appendChild($xxxxxxElement, $this->_formatsElement($articleFile));
		...
		if (!empty($rights)) {
			...
			XMLCustomWriter::appendChild($xxxxxxElement, $rightsListElement);
		}

		// Descriptions
		$descriptionsElement =& $this->_descriptionsElement($issue, $article, $suppFile, $objectLocalePrecedence, $articlesByIssue);
		if ($descriptionsElement) XMLCustomWriter::appendChild($xxxxxxElement, $descriptionsElement);
