...
class DOAJExportDom {
	...
	function &generateArticleDom(&$doc, &$journal, &$issue, &$section, &$article) {
		$xxxxxx =& XMLCustomWriter::createElement($doc, 'record');

		/* --- Article Language --- */
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'language', DOAJExportDom::mapLang($article->getLanguage()), false);

		/* --- Publisher name (i.e. institution name) --- */
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'publisher', $journal->getSetting('publisherInstitution'), false);

		/* --- Journal's title --- */
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'journalTitle', $journal->getLocalizedTitle(), false);

		/* --- Identification Numbers --- */
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'issn', $journal->getSetting('printIssn'), false);
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'eissn', $journal->getSetting('onlineIssn'), false);

		/* --- Article's publication date, volume, issue, DOI --- */
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'publicationDate', DOAJExportDom::formatDate($article->getDatePublished()), false);

		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'volume',  $issue->getVolume(), false);

		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'issue',  $issue->getNumber(), false);
		...
		if (preg_match("/([0-9]+)\s*-\s*([0-9]+)/i", $pages, $matches)) {
			...
			XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'startPage', $matches[1]);
			XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'endPage', $matches[2]);
		} elseif (preg_match("/(e[0-9]+)/i", $pages, $matches)) {
			// elocation-id (eg. "e12")
			XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'startPage', $matches[1]);
			XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'endPage', $matches[1]);
		}

		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'doi',  $article->getPubId('doi'), false);

		/* --- Article's publication date, volume, issue, DOI --- */
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'publisherRecordId',  $article->getPublishedArticleId(), false);

		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'documentType',  $article->getLocalizedType(), false);

		/* --- Article title --- */
		foreach ((array) $article->getTitle(null) as $locale => $title) {
			if (empty($title)) continue;

			$titleNode =& XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'title', $title);
		...
		XMLCustomWriter::appendChild($xxxxxx, $authors);

		$affilList = DOAJExportDom::generateAffiliationsList($article->getAuthors());

		foreach ($article->getAuthors() as $author) {
			$authorNode =& DOAJExportDom::generateAuthorDom($doc, $xxxxxx, $issue, $article, $author, $affilList);
		...
		if (!empty($affilList[0])) {
			...
			XMLCustomWriter::appendChild($xxxxxx, $affils);
		...
		foreach ((array) $article->getAbstract(null) as $locale => $abstract) {
			...
			$abstractNode =& XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'abstract', $abstract);
			if (strlen($locale) == 5) XMLCustomWriter::setAttribute($abstractNode, 'language', DOAJExportDom::mapLang(String::substr($locale, 0, 2)));
		}

		/* --- FullText URL --- */
		$fullTextUrl =& XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'fullTextUrl', Request::url(null, 'article', 'view', $article->getId()));
		XMLCustomWriter::setAttribute($fullTextUrl, 'format', 'html');

		/* --- Keywords --- */
		$keywords =& XMLCustomWriter::createElement($doc, 'keywords');
		XMLCustomWriter::appendChild($xxxxxx, $keywords);
	...
	function &generateAuthorDom(&$doc, &$journal, &$issue, &$article, &$author, &$affilList) {
		...
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'name', $author->getFullName());
		XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'email', $author->getEmail(), false);

		if(in_array($author->getLocalizedAffiliation(), $affilList)  && !empty($affilList[0])) {
			XMLCustomWriter::createChildWithText($doc, $xxxxxx, 'affiliationId', current(array_keys($affilList, $author->getLocalizedAffiliation())));
		}
