...
class EruditExportDom {
	function &generateArticleDom(&$doc, &$journal, &$issue, &$article, &$galley) {
		...
		XMLCustomWriter::setAttribute($xxxxxx, 'idprop', $journal->getId() . '-' . $issue->getId() . '-' . $article->getId() . '-' . $galley->getId(), false);
		XMLCustomWriter::setAttribute($xxxxxx, 'arttype', 'article');

		$lang = $article->getLanguage();
		XMLCustomWriter::setAttribute($xxxxxx, 'lang', isset($lang)?$lang:'en');
		XMLCustomWriter::setAttribute($xxxxxx, 'processing', 'cart');

		/* --- admin --- */

		$adminNode =& XMLCustomWriter::createElement($doc, 'admin');
		XMLCustomWriter::appendChild($xxxxxx, $adminNode);
		...
		XMLCustomWriter::appendChild($xxxxxx, $frontMatterNode);
		...
		XMLCustomWriter::appendChild($xxxxxx, $bodyNode);
