...
class DOIExportDom {
	...
	//
	/**
	 * Generate the XML xxxxxx element.
	 *
	 * @return XMLNode|DOMImplementation
	 */
	function &xxxxxxElement() {
		// Create the xxxxxx element and make it the document element of the document.
		$xxxxxxElement =& XMLCustomWriter::createElement($this->getDoc(), $this->getRootElementName());

		// Add xxxxxx-level attributes.
		XMLCustomWriter::setAttribute($xxxxxxElement, 'xmlns', $this->getNamespace());
		XMLCustomWriter::setAttribute($xxxxxxElement, 'xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance');
		XMLCustomWriter::setAttribute($xxxxxxElement, 'xsi:schemaLocation', $this->getXmlSchemaLocation());

		return $xxxxxxElement;
	}
