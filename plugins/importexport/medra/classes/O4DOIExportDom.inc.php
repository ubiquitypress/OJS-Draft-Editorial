...
class O4DOIExportDom extends DOIExportDom {
	...
	function &generate(&$objects) {
		...
		XMLCustomWriter::appendChild($xxxxxxElement, $headerElement);

		// Generate and add the O4DOI payload.
		foreach ($objects as $object) {
			if (!($objectElement =& $this->_objectElement($object))) return $falseVar;
			XMLCustomWriter::appendChild($xxxxxxElement, $objectElement);
	...
	function getRootElementName() {
		return $this->_getSchemaInfo('xxxxxxElementName');
	}
	...
	function _setSchemaInfo($schema) {
		static $schemaInfos = array(
			O4DOI_ISSUE_AS_WORK => array(
				'xxxxxxElementName' => 'ONIXDOISerialIssueWorkRegistrationMessage',
			...
			O4DOI_ISSUE_AS_MANIFESTATION => array(
				'xxxxxxElementName' => 'ONIXDOISerialIssueVersionRegistrationMessage',
			...
			O4DOI_ARTICLE_AS_WORK => array(
				'xxxxxxElementName' => 'ONIXDOISerialArticleWorkRegistrationMessage',
			...
			O4DOI_ARTICLE_AS_MANIFESTATION => array(
				'xxxxxxElementName' => 'ONIXDOISerialArticleVersionRegistrationMessage',
