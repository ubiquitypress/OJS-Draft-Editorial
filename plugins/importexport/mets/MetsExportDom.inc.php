...
class MetsExportDom {
	  ...
	  */
	function generateStructMap(&$doc, &$xxxxxx, &$journal , &$issues) {
		...
		XMLCustomWriter::appendChild($xxxxxx, $structMap);
	}

	function generateIssueDiv(&$doc, &$xxxxxx, &$issue) {
		...
		XMLCustomWriter::appendChild($xxxxxx, $pDiv);
	}

	function generateSectionDiv(&$doc, &$xxxxxx, &$section, &$issue) {
		$pDiv =& XMLCustomWriter::createElement($doc, 'METS:div');
	 ...
	 */
	function generateArticleDiv(&$doc, &$xxxxxx, &$article, &$issue) {
		$pDiv =& XMLCustomWriter::createElement($doc, 'METS:div');
	 ...
	 */
	function generateArticleFileDiv(&$doc, &$xxxxxx, $file) {
		...
		XMLCustomWriter::appendChild($xxxxxx, $fDiv);
	}

	function generateArticleHtmlGalleyImageFileDiv(&$doc, &$xxxxxx, &$imageFile, &$article){
		$fDiv =& XMLCustomWriter::createElement($doc, 'METS:fptr');
	 ...
	 */
	function generateArticleSuppFilesDiv(&$doc, &$xxxxxx, $suppFile) {
		$sDiv =& XMLCustomWriter::createElement($doc, 'METS:div');
		...
		XMLCustomWriter::appendChild($xxxxxx, $sDiv);
	}

	/**
	 *  creates the METS:dmdSec element for the Journal
	 */
	function generateJournalDmdSecDom(&$doc, $xxxxxx, &$journal) {
		...
		XMLCustomWriter::setAttribute($xxxxxx, 'xsi:schemaLocation', str_replace(' http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-0.xsd', '', $xxxxxx->getAttribute('xsi:schemaLocation')) . ' http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-0.xsd');
		...
		XMLCustomWriter::appendChild($xxxxxx, $dmdSec);
	}

	/**
	 *  creates the METS:dmdSec element for an Issue
	 */
	function generateIssueDmdSecDom(&$doc, &$xxxxxx, &$issue, &$journal) {
		$dmdSec =& XMLCustomWriter::createElement($doc, 'METS:dmdSec');
		...
		$i = 0;
		while ($i < sizeof($sectionArray)) {
			MetsExportDom::generateSectionDmdSecDom($doc, $xxxxxx, $sectionArray[$i], $issue, $journal);
			$i++;
		}
	}
	 ...
	 */
	function generateSectionDmdSecDom(&$doc, &$xxxxxx, &$section, &$issue, &$journal) {
		$dmdSec =& XMLCustomWriter::createElement($doc, 'METS:dmdSec');
		...
		$i = 0;
		while ($i < sizeof($publishedArticleArray)) {
			MetsExportDom::generateArticleDmdSecDom($doc, $xxxxxx, $publishedArticleArray[$i], $issue, $journal);
			$i++;
		}
	}
	 ...
	 */
	function generateArticleDmdSecDom(&$doc, &$xxxxxx, &$article, &$issue, &$journal) {
		$dmdSec =& XMLCustomWriter::createElement($doc, 'METS:dmdSec');
		...
		$i = 0;
		while ($i < sizeof($suppFilesArray)) {
			MetsExportDom::generateArticleSuppFilesDmdSecDom($doc, $xxxxxx, $suppFilesArray[$i]);
			$i++;
		}
	}
	 ...
	 */
	function generateArticleSuppFilesDmdSecDom(&$doc, &$xxxxxx, $suppFile) {
		$dmdSec =& XMLCustomWriter::createElement($doc, 'METS:dmdSec');
	 ...
	 *
	 */
	function generateIssueFileSecDom(&$doc, &$xxxxxx, &$issue, &$journal) {
		$publishedArticleDao =& DAORegistry::getDAO('PublishedArticleDAO');
		$publishedArticleArray = $publishedArticleDao->getPublishedArticles($issue->getId());
		$i = 0;
		while ($i < sizeof($publishedArticleArray)) {
			MetsExportDom::generateArticleFilesDom($doc, $xxxxxx, $publishedArticleArray[$i], $issue, $journal);
			$i++;
		}
	}

	function generateIssueHtmlGalleyFileSecDom(&$doc, &$xxxxxx, &$issue, &$journal) {
		$publishedArticleDao =& DAORegistry::getDAO('PublishedArticleDAO');
		$publishedArticleArray = $publishedArticleDao->getPublishedArticles($issue->getId());
		$i = 0;
		while ($i < sizeof($publishedArticleArray)) {
			MetsExportDom::generateArticleHtmlGalleyFilesDom($doc, $xxxxxx, $publishedArticleArray[$i], $issue, $journal);
			$i++;
		}
	}
	 ...
	 */
	function generateArticleFilesDom(&$doc, &$xxxxxx, &$article, &$issue, &$journal) {
		$articleGalleyDao =& DAORegistry::getDAO('ArticleGalleyDAO');
		$i = 0;
		$galleysArray =& $articleGalleyDao->getGalleysByArticle($article->getId());
		while ($i < sizeof($galleysArray)) {
			if(!$galleysArray[$i]->isHTMLGalley())
				MetsExportDom::generateArticleFileDom($doc, $xxxxxx, $article, $galleysArray[$i], null, $journal);
			$i++;
		}
		$suppFileDao =& DAORegistry::getDAO('SuppFileDAO');
		$suppFilesArray =& $suppFileDao->getSuppFilesByArticle($article->getId());
		$i = 0;
		while ($i < sizeof($suppFilesArray)) {
			MetsExportDom::generateArticleSuppFileDom($doc, $xxxxxx, $article, $suppFilesArray[$i], $journal);
			$i++;
		}
	}

	function generateArticleHtmlGalleyFilesDom(&$doc, &$xxxxxx, &$article, &$issue, &$journal) {
		$articleGalleyDao =& DAORegistry::getDAO('ArticleGalleyDAO');
		$i = 0;
		$galleysArray =& $articleGalleyDao->getGalleysByArticle($article->getId());
		while ($i < sizeof($galleysArray)) {
			if($galleysArray[$i]->isHTMLGalley()){
				MetsExportDom::generateArticleFileDom($doc, $xxxxxx, $article, $galleysArray[$i], 'html', $journal);
				$images = $galleysArray[$i]->getImageFiles();
				foreach ($images as $image) {
					MetsExportDom::generateArticleHtmlGalleyImageFileDom($doc, $xxxxxx, $article, $galleysArray[$i], $image, 'html', $journal);
				}
			}
			$i++;
		}
	...
	function generateArticleHtmlGalleyImageFileDom(&$doc, &$xxxxxx, &$article, &$galley, &$imageFile, $useAttribute, &$journal) {
		...
		XMLCustomWriter::appendChild($xxxxxx, $mfile);
	}
	 ...
	 */
	function generateArticleFileDom(&$doc, &$xxxxxx, &$article, &$galleyFile, $useAttribute, &$journal) {
		import('classes.file.PublicFileManager');
		import('lib.pkp.classes.file.FileManager');
		$fileManager = new FileManager();
		$contentWrapper = Request::getUserVar('contentWrapper');
	 ...
	 *  checks if METS:FContent or METS:FLocat should be used
	 */
	function generateArticleSuppFileDom(&$doc, &$xxxxxx, &$article, &$suppFile, &$journal) {
		import('classes.file.PublicFileManager');
		import('lib.pkp.classes.file.FileManager');
		$fileManager = new FileManager();
		$contentWrapper = Request::getUserVar('contentWrapper');
	 ...
	 */
	function &createmetsamdSec($doc, &$xxxxxx, &$journal) {
		...
		XMLCustomWriter::setAttribute($xxxxxx, 'xsi:schemaLocation', str_replace(' http://www.loc.gov/standards/premis/v1 http://www.loc.gov/standards/premis/v1/PREMIS-v1-1.xsd', '', $xxxxxx->getAttribute('xsi:schemaLocation')) . ' http://www.loc.gov/standards/premis/v1 http://www.loc.gov/standards/premis/v1/PREMIS-v1-1.xsd');
	...
	function &createmetsHdr($doc) {
		$xxxxxx =& XMLCustomWriter::createElement($doc, 'METS:metsHdr');
		XMLCustomWriter::setAttribute($xxxxxx, 'CREATEDATE', date('c'));
		XMLCustomWriter::setAttribute($xxxxxx, 'LASTMODDATE', date('c'));
		...
		XMLCustomWriter::appendChild($xxxxxx, $agentNode);
		...
		XMLCustomWriter::appendChild($xxxxxx, $agentNode2);
		return $xxxxxx;
	}
