...
class METSExportPlugin extends ImportExportPlugin {
	...
	function exportIssues(&$journal, &$issues){
		...
		$xxxxxx =& XMLCustomWriter::createElement($doc, 'METS:mets');
		XMLCustomWriter::setAttribute($xxxxxx, 'xmlns:METS', 'http://www.loc.gov/METS/');
		XMLCustomWriter::setAttribute($xxxxxx, 'xmlns:xlink', 'http://www.w3.org/TR/xlink');
		XMLCustomWriter::setAttribute($xxxxxx, 'xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance');
		XMLCustomWriter::setAttribute($xxxxxx, 'PROFILE', 'Australian METS Profile 1.0');
		XMLCustomWriter::setAttribute($xxxxxx, 'TYPE', 'journal');
		XMLCustomWriter::setAttribute($xxxxxx, 'OBJID', 'J-'.$journal->getId());
		XMLCustomWriter::setAttribute($xxxxxx, 'xsi:schemaLocation', 'http://www.loc.gov/METS/ http://www.loc.gov/mets/mets.xsd');
		$headerNode =& MetsExportDom::createmetsHdr($doc);
		XMLCustomWriter::appendChild($xxxxxx, $headerNode);
		MetsExportDom::generateJournalDmdSecDom($doc, $xxxxxx, $journal);
		...
		foreach ($issues as $issue) {
			MetsExportDom::generateIssueDmdSecDom($doc, $xxxxxx, $issue, $journal);
		...
		$amdSec =& MetsExportDom::createmetsamdSec($doc, $xxxxxx, $journal);
		XMLCustomWriter::appendChild($xxxxxx, $amdSec);
		XMLCustomWriter::appendChild($fileSec, $fileGrpOriginal);
		XMLCustomWriter::appendChild($fileSec, $fileGrpDerivative);
		XMLCustomWriter::appendChild($xxxxxx, $fileSec);
		MetsExportDom::generateStructMap($doc, $xxxxxx, $journal, $issues);
