...
class PubIdImportExportPlugin extends ImportExportPlugin {
	...
	/**
	 * Get the name of the xxxxxx node of the xml document.
	 * @return string
	 */
	...
	function handleImport(&$context, &$doc, &$errors, &$pubIds, $isCommandLine) {
		...
		$xxxxxxNodeName = $this->getRootNodeName($doc);

		switch ($xxxxxxNodeName) {
			case 'pubIds':
				$this->importPubIds($journal, $doc->children, $pubIds, $errors, $isCommandLine);
				break;
			default:
				$errors[] = array('plugins.importexport.pubIds.import.error.unsupportedRoot', array('xxxxxxName' => $xxxxxxNodeName));
				break;
		}
	}
